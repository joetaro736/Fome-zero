...
with st.container():
    ...
    with col1:
        ...
        df_aux = (df.loc[:, ['Votes', 'City']].groupby('City', sort=True).mean().reset_index().sort_values(by='Votes', ascending=True))
        linha = df_aux['Votes'] > 3.0
        df_aux = df_aux.loc[linha, ['City', 'Votes']]
        fig = px.bar(df_aux.head(7), x='City', y='Votes')
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        st.subheader('Cidades com Restaurantes com média de avaliação abaixo de 3')
        df_aux = (df.loc[:, ['Votes', 'City']].groupby('City', sort=False).mean().reset_index().sort_values(by='Votes', ascending=False))
        linha = df_aux['Votes'] < 3.0
        df_aux = df_aux.loc[linha, ['City', 'Votes']]
        fig = px.bar(df_aux.head(7), x='City', y='Votes')
        st.plotly_chart(fig, use_container_width=True)

st.subheader('Top 10 Cidades com amis tipos de culinárias diferentes')
